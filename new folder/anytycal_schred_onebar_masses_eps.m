...
%     X=1;
    ...
    xlabel('x,нм')
    ylabel('E, эВ')
    text(-5, 0.4, sprintf('z = %0.2f нм% ', b*1e9),"FontSize",12,"FontWeight","bold")
    ...
    ylim([0 Umax])
    xlim([0 1])
    legend('m_{действ}','m_{ср}', 'Location','northwest')
    box on
...
%     x=linspace(zlim(1),zlim(2),N)*1e-9;
    ...
    text(0.4, 0.3, sprintf('E_{fact} = %0.2f эВ', Efact(ind)),"FontSize",12,"FontWeight","bold")
    yline(Efact(ind),'HandleVisibility','off')
    text(0.4, 0.2, sprintf('E_{aver} = %0.2f эВ', Eaver(ind)),"FontSize",12,"FontWeight","bold")
...
ylabel('eps, %')
xlabel('x')
title({'Погрешность экстремума в зависимости', 'от высоты барьера для b=5 нм','при фиксированной доле замещающего материала x=0.3'})
