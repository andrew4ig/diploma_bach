...
v = VideoWriter('TwoBarX.mp4','MPEG-4');
v.FrameRate = 4;
v.Quality = 100;
open(v);
...
%     En=Emax(1);
    Eupper=linspace(Emax(1)*0.5,0.5*e,N);
    Dupper=zeros(1,N);
    for i=1:N
        T=Koef(Eupper(i));
        Dupper(i)=abs(T(8))^2;
...
xlabel('X')
title({'Зависмость резонасного коэффициента прохождения', 'от доли замещающего материала каналов'})
