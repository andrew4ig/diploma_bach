...
D=zeros(1,Nen);
D1=zeros(1,Nen);
J=zeros(1,length(V)); JJ=J;

v = VideoWriter('veryfVCC1.avi','Motion JPEG AVI');
v.Quality = 95;
v.FrameRate=5;
open(v);
...
for klo=1:length(V)
    ...
    for ii=1:Nen
        T1=Tline(En(ii));
        DD(ii)=abs(g(En,m1,-V(klo)*e))/abs(g(En,m1,0))*mm(1)/mm(end)*abs((T1(4)*T1(1)-T1(2)*T1(3))/T1(4))^2;
    end
...
grid on
title('ВАХ')
[value,b]=max(abs(JJ-J));
xline(V(b),'-.','HandleVisibility','off')
plot([V(b),V(b)], [JJ(b),J(b)],'or','HandleVisibility','off')


text(0.275, 1.75e10, [sprintf('(I_1-I_2)_{max}=%.2f',value/J(b)*100),'%'])

% %%
% max(abs(DD-Danalyt)/DD)*100                   %максимальное расхождение коэффициента прозрачности по ММП 
% max(abs(D-Danalyt)/D)*100                     %максимальное расхождение коэффициента прозрачности по МФГ
% 
