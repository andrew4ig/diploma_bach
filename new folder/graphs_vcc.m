...
Nw0=0;
[t,Nw]=ode15s(@(t,nw)eq(nw,t),[0 2/v],Nw0,odeset('Mass',10,'RelTol',1e-10));
...
% index=[1 55 63 71 88 112];
index=round(linspace(1,length(t)/2.3,6));
for i=1:6
    subplot(2,3,i)
    plot(x,y)                                   %structure
    hold on
...
J=[J;0];
close all
f=figure('Units','normalized','OuterPosition',[0 0 0.2 0.4]);
% subplot(2,2,1)
% plot(t*1e6,Nw)
% grid on
...
title('ВАХ','FontSize',14)
xlim([0 1])
ylim([0 1.5e-4])
text(0.65,1.3e-4, sprintf('f = 10^{%g} Гц',log(v)/log(10)),'FontSize',14)
% text(0.65,13e-5, sprintf('f = %g Гц',v),'FontSize',14)
exportgraphics(f,[num2str(log(v)/log(10)),'.jpg'])