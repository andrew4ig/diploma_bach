...
v = VideoWriter('TwoBarX12.mp4','MPEG-4');
v.FrameRate = 4;
v.Quality = 100;
open(v);
...
for ind=1:length(Xval)
    ...
    plot(xnew*1e9, Unew,'-.k','LineWidth',2,'HandleVisibility','off')
    xlim([x(1) x(end)]*1e9)
    ylim([0 Umax])
...
%     x=linspace(-5e-9,25e-9,Nx);%b+b+a+5e-9
    ...
    xnew=[-b,   0,  0,  b,  b, a+b, a+b,    a+b+b2, a+b+b2, a+2*b+b2];
    Unew=[0     0   U1  U1  0   0   U2      U2      0       0]/e;
    plot(xnew*1e9, Unew,'-b','LineWidth',1,'HandleVisibility','off')
    xlim([x(1) x(end)]*1e9)
    ylim([0 Umax])
    yline(En/e,'Color',[0.7 0.7 0.7]);
    xlabel('x,нм')
...
% xlabel('X')
% title({'Зависмость резонасного коэффициента прохождения', 'от доли замещающего материала второго барьера'})
